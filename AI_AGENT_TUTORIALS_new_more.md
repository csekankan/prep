# AI Agent Development — Complete Tutorial Reference (Beginner → Staff Engineer)
...
> **New to AI agents?** Read [Section 0](#0-beginner-primer--read-this-first) first, and see the GENAI_PYTHON_BACKEND doc's Section 0 for even more foundational vocabulary.
...
### What makes this doc different from GENAI_PYTHON_BACKEND.md?

That other doc is a broad REFERENCE covering every GenAI concept individually (RAG, agents, MCP, etc.
each in isolation). This doc is three complete, runnable, END-TO-END PROJECTS that combine several of
those concepts together into a real working system — read GENAI_PYTHON_BACKEND.md first if a term here
(RAG, MCP, tool calling, hallucination) is unfamiliar, then come back here to see it applied in context.

### The three projects at a glance

```
Project 1 — MCP Customer Support Agent: an agent that uses MCP tools (Section 10 of the GenAI doc)
            to look up order status, issue refunds, etc. — the "tool use" concept applied to a
            realistic support-bot scenario.

Project 2 — Agentic RAG Knowledge Base: not just plain RAG (retrieve once, answer) but an AGENT that
            can decide to search AGAIN with a refined query if the first search didn't find enough,
            or search multiple different sources — RAG (GenAI doc Section 6) + Agents (Section 9)
            combined.

Project 3 — Hallucination & Reliability: patterns for detecting and reducing when the model confidently
            states something false, and for measuring how reliable a GenAI system actually is —
            directly relevant to the "Evaluation" concept in the GenAI doc's Section 15.
```

Each project is self-contained and runnable — Section 8 covers how to actually set them up and run them
locally.
