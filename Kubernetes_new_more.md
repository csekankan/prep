# Kubernetes for Beginners — Zero to Confident (Beginner → Staff Engineer)
...
> This doc assumes you already know Docker basics (an "image" is a packaged app; a "container" is a
> running instance of an image). If that's unfamiliar, learn basic Docker first — this guide starts
> exactly where Docker leaves off: "now I have containers, how do I run LOTS of them reliably?"
...
**Beginner recap:** You never actually tell Kubernetes "run this container" directly — you always
wrap it in a Pod first, even for a single container. Think of a Pod as a thin envelope Kubernetes uses
to schedule and track containers as one unit; the reason it CAN hold multiple containers (rare, but
useful for "helper" containers like a log-shipper sitting alongside your main app) is exactly why it
exists as its own concept instead of Kubernetes just working with raw containers directly.
...
apiVersion: v1        # which version of the Kubernetes API this object's shape follows —
                       # "v1" is the stable, foundational API group that Pods belong to
kind: Pod              # what KIND of object this YAML describes — tells kubectl which
                       # rules/fields to expect below (a Deployment or Service YAML would
                       # have a totally different `spec` shape)
metadata:
  name: my-first-pod    # the unique name this Pod will be known by inside the cluster —
                        # this is what shows up when you run `kubectl get pods`
  labels:
    app: hello          # an arbitrary key-value tag — NOT used by Pods themselves, but this
                        # is EXACTLY the mechanism Services (Section 6) use to find "which
                        # Pods should I send traffic to?" (see Section 14, Labels & Selectors)
spec:                   # the actual DESIRED STATE — "here's what I want this Pod to run"
  containers:
  - name: web            # a name for this container WITHIN the pod (matters if there are
                          # multiple containers in one Pod, for logs/exec targeting)
    image: nginx:latest   # which container image to run — pulled from a registry (Docker Hub
                          # by default) the same way `docker run nginx:latest` would
    ports:
    - containerPort: 80    # documents that this container listens on port 80 — this is
                            # informational for other tools reading this YAML; it does NOT,
                            # by itself, expose the Pod to anything outside the cluster (that
                            # requires a Service, covered next in Section 6)
```
...
**Beginner recap:** You almost NEVER create a raw Pod directly in real usage (Section 4's example was
purely for learning) — you create a Deployment, and Kubernetes creates and manages the Pods FOR you.
This matters because a raw Pod that crashes just... stays dead; a Deployment notices and creates a
replacement automatically. This one difference (self-healing) is why Deployments are the real,
production way apps get run on Kubernetes.

**Used in production?** Yes — Deployments are the standard way to run stateless applications (web
servers, APIs) on essentially every production Kubernetes cluster in the world. For STATEFUL apps
(databases) there's a related-but-different object called a StatefulSet (covered in the more advanced
Kubernetes docs in this repo) — the distinction matters because stateful apps care about which specific
Pod has which specific storage/identity, while a Deployment's Pods are treated as interchangeable clones.
...
**Beginner recap:** Pods are constantly being created and destroyed (a crash triggers a NEW Pod with a
BRAND NEW IP address, per Section 5) — so nothing else in your system can reliably remember "Pod X's IP"
for very long. A Service is a stable, unchanging address that automatically routes to whichever Pods are
currently healthy, no matter how many times the underlying Pods get replaced. This is the exact
mechanism the Labels/Selectors concept from Section 4 is used for: a Service finds its target Pods by
matching labels, not by remembering specific IPs.

**Used in production?** Yes, universally — every real Kubernetes app needs Services to let its Pods be
reached reliably, both by other things inside the cluster and (via Ingress, Section 12) from the
internet.
