# TanStack Query, SWR, Zustand & React Libraries — Interview-Ready Deep Dive (Beginner → Staff Engineer)
...
### "Client state" vs "server state" — the key distinction this whole doc is built on

```
Client state:  data that only exists in the browser, created by the user's own interactions.
               e.g. "is this dropdown open?", "which tab is selected?"
               → Owned entirely by YOUR app. Nothing else can change it behind your back.

Server state:  data that actually lives on a server/database and you've merely fetched a COPY of.
               e.g. "the list of this user's orders"
               → Can go stale at any time (another user or process might change it on the server
               without your app knowing). Needs re-fetching, loading/error states, caching, and
               "is this still fresh?" logic — a fundamentally different, harder problem than client state.
```

Redux/Zustand/Context (covered later) are built for CLIENT state. TanStack Query/SWR are built
specifically for SERVER state — this is why you'll often see both used together in the same app
(e.g. Redux for UI state + TanStack Query for API data), not as competing alternatives.

### Why not just `useEffect` + `fetch` for server state?

```jsx
// The "naive" way — looks simple but is missing a LOT that TanStack Query gives you for free:
useEffect(() => {
  fetch("/api/user").then(r => r.json()).then(setUser);
}, []);

// Missing: loading state, error handling, caching (refetches EVERY time this component mounts,
// even if you just fetched the same data 2 seconds ago), no automatic retry on failure, no way
// to know when the data is "stale" and should be refreshed, no request de-duplication if two
// components fetch the same thing at once, no built-in cancel-on-unmount...
```

TanStack Query (and similarly, SWR) is a library that handles ALL of the above for you, so you write
much less code and get a more robust result than hand-rolling it yourself.

### How to read this document

Section 1 covers TanStack Query in depth (it's the most widely adopted option and a good default to
learn first); the rest of the sections cover alternatives (SWR, Zustand, Jotai) so you can see how they
compare, ending with a direct comparison table in Section 7 to help you actually choose for a new project.

---

## 1. TanStack Query (React Query)

**Beginner recap:** This is the deep-dive into the "server state" library concept introduced above —
notice as you read through how much of it (caching, retries, stale-while-revalidate, cross-tab sync) is
solving problems you likely wouldn't have thought to handle if you were writing the fetch logic by hand.
