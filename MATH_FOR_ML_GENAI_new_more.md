...
> **New to math notation?** Read [Section 0](#0-beginner-primer--read-this-first) first.
...
### Reading mathematical notation, for people who haven't touched it since school

```
Σ (capital sigma)   "sum of." Σᵢ xᵢ means "add up every xᵢ" — e.g. for i=1 to 3: x₁ + x₂ + x₃.

∂ (partial d)       "how much does this change if I nudge just ONE input, holding others fixed?"
                    ∂L/∂w means "how much does the loss L change per tiny change in weight w" —
                    this IS the "gradient" used in gradient descent (see ML docs' Section 0/1).

∇ (nabla)           "gradient" — the partial derivative with respect to EVERY variable at once,
                    bundled into a vector. Points in the direction of steepest increase.

x, y (plain)        a single number (a "scalar")
x, w (bold)         a list of numbers (a "vector") — e.g. an embedding is a vector
X, W (bold capital) a grid of numbers (a "matrix") — e.g. a neural network layer's weights

xᵢ                  "the i-th element of vector x" (subscript = "which position")
||x||               "the length/magnitude of vector x" (a single number describing its size)
x · y  (or xᵀy)     "dot product" — multiply corresponding elements and sum them; a way of
                    measuring how much two vectors "point in the same direction" (used constantly
                    in attention and embedding similarity — see Sections 7-8)
```

### Why does ML need so much math in the first place?

Every piece of data (a word, a pixel, a sound) gets converted into numbers, and "learning" is just
repeatedly adjusting those numbers to reduce error (see the gradient descent explanation in the
ML_DEEP_LEARNING_FOR_GENAI doc's Section 0). Linear algebra (Section 1) is the language for describing
big collections of numbers (vectors/matrices) and operations on them; calculus (Section 2) is the
language for describing HOW to adjust numbers to reduce error. Nearly everything else in this document
is a specific, more advanced application of those two foundations.

### How to read this document

Sections 1-2 (Linear Algebra, Calculus) are true prerequisites — read those first if any notation
above felt unfamiliar. Sections 3-5 (probability, information theory, optimization) build the next
layer. Sections 6+ apply all of it to specific ML/GenAI topics (neural nets, Transformers, embeddings) —
useful to read alongside the corresponding conceptual sections in the ML and Transformer docs.

---

## 1. Linear Algebra

**Beginner recap:** This is where "a vector is just a list of numbers" (introduced in the primer above)
gets formalized with actual operations (addition, dot products, matrix multiplication) and the
geometric intuition behind each one — the mathematical foundation almost everything else in ML sits on.
