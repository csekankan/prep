# Data Engineering & Big Data — Staff Engineer Reference (Beginner → Staff Engineer)
...
> **New to data engineering?** Read [Section 0](#0-beginner-primer--read-this-first) first.
...
### What is "data engineering," as distinct from regular backend engineering?

```
A normal backend engineer builds APIs that handle ONE request at a time, from ONE user, reading/
writing a database sized in gigabytes to low terabytes, where response time (milliseconds) matters most.

A data engineer builds pipelines that move and transform MASSIVE volumes of data (many terabytes to
petabytes) in bulk — e.g. "every night, combine yesterday's orders, page views, and support tickets
into one big analytics table" — where throughput (how much data per hour) matters more than the
latency of any single record, and the data usually doesn't fit on one machine at all.
```

### Why can't you just use a normal database for this?

A single Postgres server, however powerful, eventually can't hold or process petabyte-scale data fast
enough — queries that touch billions of rows would take hours on one machine. The tools in this doc
(Spark, Kafka, data warehouses) exist specifically to SPREAD both the data AND the computation across
many machines working together, so a query that would take hours on 1 machine takes minutes across 100.

### The two fundamentally different "shapes" of data pipelines

```
Batch processing (Spark, Section 4):
  Process a big, FINITE chunk of data all at once, on a schedule.
  e.g. "every night at 2am, reprocess all of yesterday's orders."
  Optimizes for: throughput, handling huge volumes efficiently. Latency: minutes to hours.

Stream processing (Kafka + Flink, Section 5):
  Process data continuously, as an unbounded, never-ending stream of individual events, as they arrive.
  e.g. "the instant an order is placed, immediately update the live dashboard."
  Optimizes for: low latency (seconds or less). Handles data that never "finishes."
```

Most real data platforms use BOTH — batch for big historical reprocessing/analytics, streaming for
anything that needs to be near-real-time — which is why this doc covers both in depth.

### Key vocabulary you'll see constantly

```
Partitioning    Splitting a big dataset into smaller pieces (by date, region, etc.) so a query only
                needs to scan the relevant pieces instead of the whole dataset.

Shuffle         Spark/distributed-processing term for redistributing data across machines mid-
                computation (e.g. for a GROUP BY) — usually the most expensive part of a big job,
                because it involves sending data over the network between machines.

Schema          The structure of your data (column names/types). "Schema-on-write" (traditional DBs)
                validates structure when data is SAVED; "schema-on-read" (data lakes) stores raw data
                and only interprets structure when it's LATER read/queried — more flexible, less safe.

ETL / ELT       Extract-Transform-Load vs Extract-Load-Transform — whether you clean/reshape data
                BEFORE loading it into your warehouse, or load it raw and transform it there
                afterward (the modern default, enabled by cheap warehouse compute — see dbt, Section 10).
```
