# Frontend System Design — Advanced FAANG Reference Guide (Beginner → Staff Engineer)
...
### How is "frontend system design" different from a normal coding interview?

```
A normal frontend coding interview: "Build a working todo list component" — you write actual code,
in a fixed short amount of time, for one specific isolated feature.

Frontend SYSTEM DESIGN: "Design Instagram Stories" — much more open-ended, no single "correct" answer,
and mostly a DISCUSSION (with diagrams and pseudocode, not a fully working app) about how you'd
architect the whole feature: what components exist, how data flows in, how it scales to millions of
users, what happens on a bad network connection, etc. It tests breadth of judgment across many
concerns (performance, caching, real-time updates, accessibility) rather than depth on one algorithm.
```

### Why this document is organized the way it is

Sections 3-25 are a reference catalog of BUILDING BLOCKS (rendering, state management, caching,
real-time systems, security, etc.) — each is a tool you might reach for. Sections 26-35 are full worked
example problems ("Design Google Docs," "Design a Notification System") that show how to actually
COMBINE those building blocks to answer a real interview question end-to-end. The recommended order:
skim Section 2 (the general framework) first, then read a couple of the worked examples (26+) to see
the framework in action, using the building-block sections as reference whenever a worked example
mentions a concept you don't recognize yet.

### If you're not preparing for an interview

Everything here is equally useful as an actual architecture reference for building real frontend
systems — "interview" framing aside, the underlying engineering concerns (how do you keep a feed
fast with 10,000 items, how do you handle a flaky network, how do collaborative edits merge safely)
are exactly the same problems real production frontend teams solve daily.

---

## 1. How Frontend System Design Interviews Work

**Beginner recap:** Sets expectations for what's actually being evaluated in this kind of interview —
useful context even if you're reading this doc purely for the engineering knowledge and skipping the
interview-specific framing.
