# Monorepo Architecture — Deep Dive (Beginner → Staff Engineer)
...
### "Repo," in case that's unfamiliar

```
A "repo" (repository) is a folder tracked by a version control system (almost always Git) — it's
where your project's code, and its full history of changes, actually lives (e.g. what you'd see on
GitHub as one project).
```

### Monorepo vs. Polyrepo — the actual question this whole doc answers

```
Polyrepo (the "default" most people start with):
  repo-A (frontend)   repo-B (backend-api)   repo-C (shared-ui-library)
  Each is its OWN separate Git repository, with its own history, its own place to clone from.

Monorepo:
  ONE single repo containing frontend/, backend-api/, shared-ui-library/ all together, as
  subfolders, sharing ONE Git history.
```

Neither is "better" universally — it's a genuine trade-off (covered in depth in Section 2). The rough
intuition: monorepos make it trivial to share code between projects and make sweeping changes across
everything at once (rename a function used by 10 different apps — one commit, one PR), at the cost of
needing MORE tooling to keep things fast as the repo grows large (this doc's Sections 9-15 on task
orchestration, affected detection, and caching exist specifically to solve problems that ONLY show up
once you've gone the monorepo route).

### Why would sharing code between projects be hard WITHOUT a monorepo?

```
In a polyrepo world, if `shared-ui-library` needs a fix, you'd:
1. Make the fix in the shared-ui-library repo, publish a new package version (e.g. to npm)
2. Go update frontend's package.json to point at that new version, reinstall, test, commit
3. Repeat step 2 for EVERY other repo that depends on shared-ui-library

In a monorepo, step 1 and 2 collapse into ONE atomic change/commit — every consumer of
shared-ui-library sees the fix instantly, in the same PR, with no publish step required.
```
...
**Beginner recap:** Formalizes the definition above with precise boundaries — importantly, a monorepo
is NOT the same thing as "one giant undifferentiated codebase with no internal structure"; it's
multiple genuinely separate, independently-versioned projects/packages that just happen to share one
Git repository and toolchain.
