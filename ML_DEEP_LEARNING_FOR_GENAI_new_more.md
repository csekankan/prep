# ML & Deep Learning for GenAI Engineers — Complete Guide (Beginner → Staff Engineer)
...
> **New to ML?** Read [Section 0](#0-beginner-primer--read-this-first) first.
...
### What is "machine learning," at the most basic level?

```
Traditional programming: YOU write explicit rules.
  "if temperature > 30: print('hot')"

Machine learning: the COMPUTER finds the rules itself, by looking at many examples.
  Show it 10,000 (temperature, "hot"/"cold" label) pairs, and it learns a rule/pattern on its own
  that lets it predict "hot" or "cold" for a NEW temperature it's never seen.

"Training" = the process of the computer discovering that pattern from example data.
"Model" = the resulting learned pattern (in deep learning, literally millions/billions of numbers —
          "weights" — that combine to make predictions).
```

### Why does this doc go from "classical ML" all the way to "GPT"?

Modern LLMs (GPT, Claude, etc.) are built from ideas that were developed incrementally over decades —
linear/logistic regression (Section 1) → neural networks (Section 2) → CNNs/RNNs (Sections 4-5) →
attention (Section 6) → Transformers (Section 7) → today's massive LLMs (Section 9). Each concept
builds on the previous one; skipping straight to "Transformers" without the earlier foundations
(gradient descent, what a "loss function" even is) makes the later, harder material much less intuitive.

### The one concept that underlies almost EVERYTHING in this document: gradient descent

```
A model makes a prediction → compare it to the correct answer → compute how WRONG it was (the "loss")
→ nudge every one of the model's internal numbers (weights) slightly in the direction that would have
made the prediction LESS wrong → repeat this millions of times over lots of examples.

That's it — that iterative "nudge weights to reduce error" loop, called gradient descent, is
THE mechanism by which every neural network in this document (including GPT-scale LLMs) learns.
Everything else (CNNs, RNNs, attention, Transformers) is about WHAT the weights are arranged INTO
(the architecture), not a different way of learning them.
```
...
**Beginner recap:** Before neural networks, "classical" ML algorithms (linear regression, logistic
regression, decision trees) already captured the core "learn from examples" idea in a much simpler
form — worth understanding first, since neural networks (Section 2) are, in a real sense, a way of
stacking simple building blocks similar to these into something far more powerful.
