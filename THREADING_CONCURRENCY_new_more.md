# Threading & Concurrency — Python + Go Mastery Guide (Beginner → Staff Engineer)
...
### Why does a program ever need more than "one thing at a time"?

Two different real problems push us toward concurrency:
1. **Waiting** — your program is idle while waiting on something slow (a network call, disk I/O) and
   could be doing OTHER useful work during that wait instead of just blocking.
2. **True speed** — you have independent CPU-heavy work (e.g. resizing 1000 images) that could
   literally run simultaneously on your machine's multiple CPU cores to finish faster.

These are different problems with different solutions — problem 1 is solved by "concurrency" broadly
(threads, async), problem 2 specifically needs "parallelism" (see below), and conflating the two is
the single most common source of confusion in this whole topic.

### The critical distinction this document builds everything on

```
Concurrency:  dealing with MULTIPLE tasks making progress over overlapping time periods — but not
              necessarily at the EXACT same instant. One chef, several pots on the stove, checking
              and stirring each one in turn — nothing is happening literally simultaneously, but all
              the dishes progress "concurrently" over the course of the evening.

Parallelism:  multiple tasks executing at the LITERAL SAME INSTANT, on separate CPU cores.
              Two chefs, each cooking their own dish, at truly the same time.

You CAN have concurrency without parallelism (single CPU core, switching between tasks rapidly) —
this is exactly what Python's asyncio and the GIL (Section 2) force you into for most Python programs.
```

### Thread vs Process, in one paragraph

A **process** is a running program with its own private memory — two processes can't accidentally
corrupt each other's data because they don't share memory at all (communication requires explicit
mechanisms, like pipes). A **thread** is a lighter-weight unit of execution WITHIN a process — all
threads in the same process SHARE the same memory, which makes communication between them easy and
fast, but also means they can step on each other's data if you're not careful (hence needing locks —
Section 3/7). This is the foundation for why Section 4 recommends different tools (threads vs
processes vs async) for different kinds of work.

---

## 1. Mental Model: Concurrency vs Parallelism

**Beginner recap:** Formalizes the concurrency-vs-parallelism distinction from the primer above with
more visual/concrete examples — this single distinction is worth being 100% solid on before continuing,
since it's the lens the rest of the document's tool choices (threads vs processes vs async) are viewed
through.
