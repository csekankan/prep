# React — Complete Core Concepts & FAANG Interview Reference (Beginner → Staff Engineer)
...
> **Brand new to React?** Read [Section 0](#0-beginner-primer--read-this-first) first.
...
### What problem does React actually solve?

```
Without a framework, updating the UI when data changes means manually finding the right DOM element
and mutating it yourself:
  document.getElementById("cart-count").textContent = newCount;

This gets unmanageable fast once a page has many interdependent pieces changing — you'd have to
remember EVERY place in the DOM that depends on "cart count" and manually update each one, every time
it changes, anywhere in your code.

React's core idea: you describe WHAT the UI should look like for the CURRENT data ("if count is 3,
render <span>3 items</span>"), and React figures out what actually needs to change in the real DOM
when that data changes. You stop manually touching the DOM entirely — you just update your data
(state) and re-describe the UI; React handles the "how do I update the actual pixels" part.
```

### The absolute basics: components, JSX, and props

```jsx
// A "component" is just a JavaScript function that returns a description of some UI.
function Greeting(props) {
  return <h1>Hello, {props.name}!</h1>;
  // this weird HTML-looking syntax inside JS is JSX — see Section 2 for the full explanation.
  // It's NOT a string and it's NOT real HTML — it compiles down to plain JS function calls
  // (React.createElement(...)) that build a description of the UI (Section 3 explains this
  // description, called the "virtual DOM," in depth).
}

// "props" are how a PARENT passes data DOWN into a child component — like function arguments:
<Greeting name="Alice" />   // props = {name: "Alice"} inside Greeting
```

### State: the data that changes, and why React needs to know about it explicitly

```jsx
function Counter() {
  const [count, setCount] = useState(0);
  // useState gives you: the CURRENT value (count), and a function to update it (setCount).
  // Crucially, you can't just do `count = count + 1` — React needs to be TOLD via setCount()
  // that something changed, so it knows to re-run this component function and update the DOM.

  return <button onClick={() => setCount(count + 1)}>Clicked {count} times</button>;
}
```

Every time `setCount` is called, React re-runs the `Counter` function to get a fresh description of
the UI, compares it to what's currently on screen (Section 3: reconciliation), and updates only the
parts of the real DOM that actually changed (in this case, just the number inside the button).

### How to read this document

Sections 1-9 are genuine fundamentals (components, JSX, virtual DOM, state, hooks) — read those in
order if you're new. From Section 10 onward, topics get progressively more specialized/advanced
(custom hooks, performance, SSR, testing) and can be read more selectively based on what you need.

---

## 1. React Fundamentals

**Beginner recap:** Formalizes everything sketched in the primer above — what React actually is, the
problem it solves relative to manually manipulating the DOM, and its core "describe the UI, let React
figure out the DOM updates" philosophy.
