# Designing Data-Intensive Applications — Key Concepts (Beginner → Staff Engineer)
...
## Beginner Primer — Read This First

### What kind of book is this, and why does it matter for backend/system-design interviews?

"Designing Data-Intensive Applications" (nicknamed "DDIA") is widely considered THE canonical book for
understanding how databases, caches, message queues, and distributed systems actually work under the
hood — not how to USE a specific database's API, but the underlying principles that are true across
almost every data system (Postgres, Kafka, Cassandra, Redis...). If you've ever wondered "why do
distributed databases have to make trade-offs at all, why isn't there just one perfect database,"
this book (and these notes) answer that directly.

### The one idea that explains almost the whole book: everything is a trade-off

A single-machine database doesn't have to worry about: what happens if the network between two of
its parts fails? What if two machines disagree about which one has the "true" latest value? The moment
you spread data across multiple machines (for scale or reliability), these problems appear, and EVERY
distributed system has to choose how to handle them — there is no configuration that avoids all the
trade-offs simultaneously. This is the deep reason behind concepts like the CP/AP trade-off you'll see
covered later in this doc, and it's worth internalizing before diving into any individual chapter.

### Some vocabulary before you start

```
Fault vs Failure    A "fault" is one component breaking (a single disk failing). A "failure" is the
                    WHOLE system going down. Good system design tolerates individual faults without
                    becoming a total failure — this distinction is the crux of Chapter 1 below.

Replication         Keeping copies of the same data on multiple machines (for reliability and to
                    spread out read load).

Partitioning        Splitting DIFFERENT pieces of data across multiple machines (so no single
                    machine has to hold or process everything) — also called "sharding."

Consistency         How up-to-date/agreed-upon a piece of data is across all its copies at any
                    given moment — the central tension explored throughout this book.
```

### How to read this document

It follows the book's own chapter structure — Part I covers foundational single-node concepts,
later parts get into distributed replication/partitioning/transactions. Reading in order builds
each concept on the last one, the way the book itself is structured.
...
**Beginner recap:** This opening chapter defines the three yardsticks (reliability, scalability,
maintainability) that every later chapter's trade-offs get measured against — worth internalizing these
definitions precisely, since "scalable" and "reliable" get used loosely in casual conversation but have
specific, distinct meanings in this book.
